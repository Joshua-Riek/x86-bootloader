;  boot12.asm
;
;  This uses a 1440k fat12 floppy disk image header   
...
;
    ...
    %ifidn __OUTPUT_FORMAT__, elf32
      BOOT_SEG    equ 0x0000                    ; Set the bootloader segments to zero when
    %else                                       ; the elf format is used (compiled with -Ttext=0x7c00),
      BOOT_SEG    equ 0x07c0                    ; which is only used for debugging
    %endif
        
    STACK_SEG     equ 0x0600                    ; (STACK_SEG * 0x10) + STACK_OFF = 0x7000
...
bootStrap:
    ...
    or dl, dl                                   ; Check to see if we are booting from a hard drive 
    jz loadRoot
...
loadFat:
    ...
    mov word [cluster], ax                      ; Load file table

    xor ax, ax                                  ; Size of fat = (fats * fat)
    mov al, byte [fats]                         ; Move number of fats into ax
    mul word [fatSectors]                       ; Move fat sectors into bx
    mov cx, ax

    mov ax, word [reservedSectors]              ; Convert the first sector of fat
    xchg bx, bx
...
loadedFat:
  ...
  loadFileSector:
    ...
    mov bx, STAGE2_SEG                          ; Segment address of the buffer
  ...
  fileJump:                                     ; Now we jump to the file
    mov dl, byte [drive]                        ; We still need the boot device info
    jmp STAGE2_SEG:STAGE2_OFF                   ; Jump to the file loaded!
...
;---------------------------------------------------
    ...
    fileNotFound   db "STAGE2.BIN not found!", 0

    userData       dw 0                         ; startOfRoot + numberOfRoot
