;  boot32.asm
;
;  This program is a FAT32 bootloader
;  Copyright (c) 2026, Joshua Riek
;
...
;---------------------------------------------------
    ...
    %define tableSize32       bp+0x24           ; Size of FAT in sectors
    %define extendedFlags     bp+0x28           ; Flags
    %define fatVersion        bp+0x2a           ; FAT version number
    %define rootCluster       bp+0x2c           ; The cluster number of the root directory
    %define fatInfo           bp+0x30           ; The sector number of the FSInfo structure
    %define backupBootSector  bp+0x32           ; The sector number of the backup boot sector
    %define reserved0         bp+0x34           ; Reserved. When formated, these bytes should be zero
    %define driveNumber       bp+0x40           ; Drive number
    %define reserved1         bp+0x41           ; Flags in Windows NT. Reserved otherwise
    %define bootSignature     bp+0x42           ; Signature value must be 0x28 or 0x29
    %define volumeId          bp+0x43           ; Volume ID 'Serial' number
    %define volumeLabel       bp+0x47           ; Volume label string
    %define fat_type_label    bp+0x52           ; System identifier string
...
loadRoot:
    ...
    xor ah, ah
    mov al, byte [fats]
    mul word [tableSize32]
    add ax, word [reservedSectors]
    mov word [startOfData], ax

    xor dx, dx                                  ; Convert FAT cluster into LBA
    mov ax, word [rootCluster]
    sub ax, 0x0002
    xor cx, cx
